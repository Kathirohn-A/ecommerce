...
const Cart =()=>{
  // const AxiosUrl="http://192.168.62.87:8081";
    ...
    const handlePlaceOrder=async (userid)=>{
       console.log("check");
            ...
            if(sortedCart.length>0){
            const res=await axios.post(`${process.env.REACT_APP_API}/placeorder`,{user:userid,products:sortedCart,total:total})
      ...
      {sortedCart.length>0?sortedCart.map((data,ind)=>{
       return(
        <div key={ind} class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
          <img src={`${process.env.REACT_APP_API}/public/images/${data.image}`} alt={data.image} class="w-full rounded-lg sm:w-40" />
