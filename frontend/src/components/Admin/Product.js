...
const Product = () => {
    ...
    const fetchData=async()=>{
      const res = await axios.get(process.env.REACT_APP_API+'/productdata')
      setView(res.data)
   }
   useEffect(()=>{
    fetchData()
   ...
   const handleSubmit= (e)=>{
        ...
        axios.post(process.env.REACT_APP_API+'/uploadDetails',formData).then(
          res=>{
  ...
  const deleteData= async(id)=>{ 
    const res= await axios.delete(`${process.env.REACT_APP_API}/deleteadminproduct/${id}`)
   ...
   const handleEditsubmit= async(id)=>{
    ...
    try{
    const res = await axios.post(`${process.env.REACT_APP_API}/editadmindata/${id}`,{name:editName,price:editprice,qty:editQty})
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    {view.length>0 ? view.map((data,ind)=>{
      ...
      <tr key={ind}  class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <img src={`${process.env.REACT_APP_API}/public/images/${data.image}`}alt={data.image} height={100} width={100}/>
